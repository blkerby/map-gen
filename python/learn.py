...
def collect_feature_mismatches(
    ...
    for field in fields(generated):
        ...
        if isinstance(generated_value, torch.Tensor):
            ...
            # Environment::clear zeros unplaced room coordinates so this exact
            # comparison cannot report stale positions from previous episodes.
...
def train_feature_batch_backward(
    ...
    if prepared_batch.kind == "fresh":
        ...
        order_rank_prices = terminal_balance_cost(
    ...
    for feature_batch in prepared_batch.feature_batches:
        ...
        if prepared_batch.kind == "fresh":
            order_balance_score_target = remaining_order_price(
                order_rank_prices, features.global_features.area_used,
            ).unsqueeze(1)
